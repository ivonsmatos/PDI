...
// Mapeamento de requisitos mínimos por passo (retorna erro ou null)
function validateStep(stepAtual: number, usuario: { nome: string }, objetivos: { id: string }[]): string | null {
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-900 font-sans transition-colors duration-300">
      ...
      <main className="flex-grow w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex flex-col">
        ...
        <div className="mb-10 relative">
          <div className="absolute top-5 left-0 w-full h-1.5 bg-gray-200 dark:bg-slate-700 -z-10 rounded-full transition-colors"></div>
          <div
            ...
            {steps.map((step) => {
              ...
              return (
                <div key={step.num} className="flex flex-col items-center relative group">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-500
                      ${isActive ? 'bg-indigo-600 dark:bg-indigo-500 text-white shadow-lg ring-4 ring-indigo-100 dark:ring-indigo-900 scale-110' :
                        isPast ? 'bg-indigo-600 dark:bg-indigo-500 text-white shadow-sm ring-2 ring-indigo-200 dark:ring-indigo-800' : 'bg-gray-100 dark:bg-slate-800 text-gray-400 dark:text-slate-500 border border-gray-300 dark:border-slate-600'}`}
                  >
                    {isPast ? (
                  ...
                  <div className={`mt-3 text-xs md:text-sm font-semibold text-center transition-colors duration-300
                      ${isActive ? 'text-indigo-700 dark:text-indigo-400' :
                        isPast ? 'text-slate-600 dark:text-slate-300' : 'text-gray-400 dark:text-slate-500'}`}
                  >
                    {step.title}
                  </div>
                </div>
        ...
        {validationError && (
          <div className="mb-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 rounded-xl px-4 py-3 text-sm font-medium flex items-center gap-2 animate-in fade-in slide-in-from-top-2 duration-300">
        ...
        <div className="mt-8 flex justify-between items-center">
          ...
          >
            {stepAtual === 6 ? 'Concluído ✓' : 'Próximo Passo'}
            {stepAtual < 6 && (
              <svg className="w-5 h-5 rtl:rotate-180" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
